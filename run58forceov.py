...
def handle_player_shoot(csv_writer=None, threshold_intensity=4):
    ...
    if len(shot_times) > 1:
        ...
        # 10% probability to trigger vibration
        if random.random() < 0.1:
            ...
            print(f"Predicted next shot at: {predicted_next_shot} ms, but no vibration triggered (10% chance)")
...
last_shot_time = 0
cooldown_time = 700
last_optimal_vibration_time = 0  # Add this line to initialize it.
...
def run_game(csv_writer, threshold_intensity):
    ...
    global vibration_times, message_display_duration, last_shot_time, cooldown_time, last_optimal_vibration_time
    ...
    while running_game:
        ...
        # --- Calculate predicted hole position ---
        food_start_x = 100
        T_travel = (wall_x - food_start_x) / food_speed
        lower_bound = wall_y
        ...
        predicted_hole_y = lower_bound + (2 * L - x_eff) if x_eff > L else lower_bound + x_eff

        optimal_min = bird_y - hole_height
        optimal_max = bird_y

        # --- Vibration in 70% of Optimal Moments ---
        if not shot_occurred and (optimal_min <= predicted_hole_y <= optimal_max):
            if current_time - last_optimal_vibration_time > 500:  # Prevents back-to-back vibrations
                if random.random() < 0.1:  # 10% probability
                    vibration_time = current_time + 50  # Adjust as needed
                    vibration_times.append(vibration_time)
                    last_optimal_vibration_time = current_time  # Update last vibration time
                    print(f"Vibration scheduled at: {vibration_time:.0f} ms (Optimal Moment)")

            # Log the optimal moment
